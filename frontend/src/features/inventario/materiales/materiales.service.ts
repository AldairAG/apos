...
export const materialesService = {
  ...
  getBySucursal: async (sucursalId: number): Promise<Material[]> => {
    const response = await apiBase.get<Material[]>(`${MATERIAL_BASE_URL}/sucursal/${sucursalId}`);
    return response.data;
  },
  ...
  getById: async (id: number): Promise<Material> => {
    const response = await apiBase.get<Material>(`${MATERIAL_BASE_URL}/${id}`);
    return response.data;
  },
  ...
  create: async (data: CreateMaterialDTO): Promise<Material> => {
    const response = await apiBase.post<Material>(MATERIAL_BASE_URL, data);
    return response.data;
  },
  ...
  update: async (id: number, data: UpdateMaterialDTO): Promise<Material> => {
    const response = await apiBase.put<Material>(`${MATERIAL_BASE_URL}/${id}`, data);
    return response.data;
  },
